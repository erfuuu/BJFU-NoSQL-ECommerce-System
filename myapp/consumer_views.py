...
def consumer_home_view(request):
    ...
    return render(request, 'consumer_home.html', {'products': products})

#搜索框
def search_products(request):
    query = request.GET.get('query', '').strip()  # 获取搜索关键字并去除首尾空格
    ...
    if query:
        ...
        if query.isdigit():
            ...
            # 如果 query 不是整数，则不进行 product_id 的过滤
            products = products.filter(
                Q(name__icontains=query) |
                Q(tags__icontains=query)
            )

    return render(request, 'consumer_home.html', {'products': products, 'query': query})
